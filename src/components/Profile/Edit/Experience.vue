...
const handleSave = () => {
  if (
    ...
    if (data.value) {
      data.value.experience.push(newExperience);
    }
...
const handleDelete = (id: number) => {
  if (data.value) {
    const index = data.value.experience.findIndex((exp) => exp.id === id);
    if (index > -1) {
...
<template>
  <div>
    ...
    <div class="mt-4">
      <div v-for="exp in data?.experience || []" :key="exp.id"
        ...
        <div class="flex justify-between items-start">
          ...
          <button @click="handleDelete(exp.id!)" class="text-red-500 hover:text-red-700 text-sm">
            Delete
          </button>
        </div>
      </div>
    ...
    <div v-if="isAdding" class="grid gap-y-4 mt-4 p-4 border border-gray-200 rounded-lg bg-gray-50">
      <input type="text" placeholder="Company Name" v-model="currentExperience.companyName"
        class="w-full border border-gray-300 rounded-lg p-2 text-sm" />
      <input type="text" placeholder="Designation" v-model="currentExperience.designation"
        class="w-full border border-gray-300 rounded-lg p-2 text-sm" />
      <input type="text" placeholder="Duration (e.g., Jan 2020 - Dec 2022)" v-model="currentExperience.duration"
        class="w-full border border-gray-300 rounded-lg p-2 text-sm" />

      <div class="flex gap-2">
        <button @click="handleSave" :disabled="!currentExperience.companyName ||
        ...
        <button @click="handleCancel" class="px-4 py-2 bg-gray-300 text-gray-700 rounded-lg text-sm hover:bg-gray-400">
          Cancel
        </button>
      </div>
    </div>

    <button v-if="!isAdding" @click="handleAddExperience"
